    ...
    <div className="bg-card border border-border rounded-lg overflow-hidden hover:border-border transition-colors">
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          <div className="flex items-center gap-2">
            <button 
              onClick={() => window.open(`https://coinmarketcap.com/currencies/${index.name.toLowerCase().replace(/\s+/g, '-')}/`, '_blank')}
      ...
      {selectedChart && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80" onClick={() => setSelectedChart(null)}>
          <div className="bg-background rounded-lg w-full max-w-4xl max-h-[90vh] overflow-hidden m-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-4">
              <CandlestickChart symbol={selectedChart.symbol} onClose={() => setSelectedChart(null)} />
            </div>
          </div>
        </div>
      )}
