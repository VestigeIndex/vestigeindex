...
export default function SwapModal({ coin, mode, feeRate = TOP100_FEE, onClose }: SwapModalProps) {
    ...
    quoteTimer.current = setTimeout(async () => {
      ...
      try {
        console.log("🔍 GETTING QUOTE FOR:", {
          ...
          amountUsd: netUsd,
          chainId: wallet.evmWallet?.chainId,
          address: wallet.evmWallet?.address
        });
        ...
        console.log("📊 QUOTE PARAMS:", {
          ...
          amountWei,
          chainId: wallet.evmWallet!.chainId,
          userAddress: wallet.evmWallet!.address,
          fee: feeRate
        });

        // Use new getSwapQuote function
        ...
        console.log("📋 TX DATA:", q.data);

        // Validate quote before setting
        if (!q || !q.data || !q.data.to || !q.data.data) {
          console.error("❌ INVALID QUOTE STRUCTURE:", q);
          setErrorMsg("Invalid quote received. Please try again.");
          setQuote(null);
          setStatus("error");
          return;
        }
  ...
  async function handleConfirmSwap() {
    ...
    try {
      if (isEVMWallet && srcToken && dstToken && wallet.evmWallet && quote) {
        console.log("💳 EVM SWAP - VALIDATING QUOTE DATA");
        console.log("QUOTE:", quote);
        console.log("TXDATA:", quote.data);

        // 🔴 CRITICAL VALIDATION - Check if quote has valid tx data
        if (!quote.data || !quote.data.to || !quote.data.data) {
          console.error("❌ INVALID QUOTE STRUCTURE - MISSING TX DATA");
          setErrorMsg("Invalid quote data. Please try again.");
          setStatus("error");
          return;
        }

        console.log("✅ QUOTE VALIDATED - EXECUTING DIRECT TRANSACTION");
        
        // Execute swap directly using signer.sendTransaction
        ...
        console.error("❌ INCOMPATIBLE WALLET");
              ...
              <div className="bg-muted rounded p-3 mb-4 text-xs space-y-1.5">
                ...
                <div className="flex justify-between border-t border-border pt-1.5">
                  ...
                  {status === "quoting" ? (
                    ...
                    <span className="font-medium">
                      ≈ {parseFloat(quote.data.outAmount).toFixed(6)} {(quote.data.outToken?.symbol || coin.symbol).toUpperCase()}
                    </span>
                  ) : (
                    <span className="font-medium">
                ...
                {quote && (
                  <div className="text-muted-foreground text-xs flex justify-between pt-1 border-t border-border">
                    ...
                    <span>{quote.data.estimatedGas?.toLocaleString() ?? "—"} gas</span>
                  </div>
                )}
